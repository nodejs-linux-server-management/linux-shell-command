...
execute('ls').then(({shellcommand: sc, success: success}) => {
    if (success === true) {
        console.log(sc.stdout);
    } else {
        console.error(sc.error);
...
try {
    //The variable "sc" is an instance of ShellCommand
    var sc = execute("cp '!?!' '!?!' -v", ['file1', 'file2'], 0, (shellCommand, success) => {
        if (success === true) {
